...
export default function Register() {
  ...
  return (
    <div className="min-h-[80vh] flex items-center justify-center px-4 relative">
       {/* Background Orbs */}
       <div className="absolute top-1/4 -right-1/4 w-96 h-96 bg-indigo-500/10 rounded-full blur-[120px] animate-pulse-slow pointer-events-none"></div>
       <div className="absolute bottom-1/4 -left-1/4 w-96 h-96 bg-primary-500/20 rounded-full blur-[120px] animate-pulse-slow pointer-events-none" style={{ animationDelay: '2s' }}></div>
      ...
      <Card variant="glass" className="max-w-md w-full !p-8 sm:!p-12 relative z-10 border-white/10 shadow-2xl">
        <div className="flex flex-col items-center mb-10">
          ...
          <h2 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight text-center">Solicitar Acesso</h2>
          <p className="mt-2 text-slate-500 dark:text-slate-400 text-sm font-bold uppercase tracking-widest text-[10px]">Criação de Conta Cliente</p>
        </div>
        ...
        <form onSubmit={handleRegister} className="space-y-5">
          ...
          <Button type="submit" className="w-full py-4 text-base shadow-lg shadow-primary-500/25 mt-4" isLoading={isLoading}>
            CRIAR MINHA CONTA
          </Button>
        </form>

        <p className="mt-10 text-center text-xs text-slate-500 dark:text-slate-500 font-bold uppercase tracking-widest">
          Já possui conta? <Link to="/login" className="text-primary-500 hover:text-primary-600 transition-colors">Fazer Login</Link>
        </p>
      </Card>
    </div>
  );
